...
DEFAULT_STRATEGIES = ["FULL", "S-FULL", "S-FIX", "ADA", "CAP", "EXPRESS", "K-EXPRESS", "EXPRESS-M"]
...
DEFAULT_CONFIG = {
    ...
    "conformal": {
        ...
        "random_calibration_track_strategy": None,
    },
...
def run_experiment(config, config_path=None, output_root=DEFAULT_RESULTS_ROOT, run_name=None):
    ...
    unknown_strategies = sorted(set(strategies) - SUPPORTED_STRATEGIES)
    if unknown_strategies:
        raise ValueError(f"Unknown strategies: {unknown_strategies}")
    k_express = int(conformal_config["k_express"])
    random_calibration_size = conformal_config.get("random_calibration_size")
    if random_calibration_size is not None:
        random_calibration_size = int(random_calibration_size)
        if random_calibration_size < 0:
            raise ValueError("conformal.random_calibration_size must be nonnegative")
    random_calibration_track_strategy = conformal_config.get(
        "random_calibration_track_strategy"
    )
    if random_calibration_track_strategy == "":
        random_calibration_track_strategy = None
    if random_calibration_track_strategy is not None:
        random_calibration_track_strategy = str(random_calibration_track_strategy)

    if "RANDOM" in strategies:
        has_fixed_random_size = random_calibration_size is not None
        has_tracked_random_size = random_calibration_track_strategy is not None
        if has_fixed_random_size == has_tracked_random_size:
            raise ValueError(
                "RANDOM requires exactly one of conformal.random_calibration_size "
                "or conformal.random_calibration_track_strategy"
            )
        if random_calibration_track_strategy == "RANDOM":
            raise ValueError("RANDOM cannot track itself")
        if (
            random_calibration_track_strategy is not None
            and random_calibration_track_strategy not in SUPPORTED_STRATEGIES
        ):
            raise ValueError(
                "conformal.random_calibration_track_strategy must be a supported "
                f"strategy, got {random_calibration_track_strategy!r}"
            )

    finite_express_target_size = conformal_config.get("finite_express_target_size")
    if finite_express_target_size is not None:
        finite_express_target_size = int(finite_express_target_size)
    finite_express_rank_delta = conformal_config.get("finite_express_rank_delta")
    if finite_express_rank_delta is not None:
        finite_express_rank_delta = float(finite_express_rank_delta)
    ...
    relaxed_express_max_distance = conformal_config.get(
        "relaxed_express_max_distance",
        0.02,
    )
    if relaxed_express_max_distance is not None:
        relaxed_express_max_distance = float(relaxed_express_max_distance)
        if relaxed_express_max_distance < 0 or relaxed_express_max_distance > 1:
            raise ValueError("relaxed_express_max_distance must be in [0, 1]")
    relaxed_express_debug = bool(conformal_config.get("relaxed_express_debug", False))
        ...
        for t in range(n_on):
            ...
            if s_t:
                ...
                evaluation_kwargs = {
                    ...
                    "k": k_express,
                    "finite_express_target_size": finite_express_target_size,
                    "finite_express_rank_delta": finite_express_rank_delta,
                    "relaxed_express_max_distance": relaxed_express_max_distance,
                    "relaxed_express_debug": relaxed_express_debug,
                ...
                def evaluate_event_strategy(strategy, preserve_rng=False):
                    if strategy in event_results:
                        return event_results[strategy]

                    target_random_calibration_size = random_calibration_size
                    if (
                        strategy == "RANDOM"
                        and random_calibration_track_strategy is not None
                    ):
                        tracked_result = evaluate_event_strategy(
                            random_calibration_track_strategy,
                            preserve_rng=True,
                        )
                        target_random_calibration_size = tracked_result["n_calibration"]

                    rng_state = None
                    if preserve_rng:
                        rng_state = copy.deepcopy(conformal.rng.bit_generator.state)

                    try:
                        strategy_result = conformal.evaluate_strategy(
                            strategy=strategy,
                            random_calibration_size=target_random_calibration_size,
                            **evaluation_kwargs,
                        )
                    finally:
                        if preserve_rng:
                            conformal.rng.bit_generator.state = rng_state

                    if not preserve_rng:
                        event_results[strategy] = strategy_result
                    return strategy_result

                for strategy in strategies:
                    strategy_result = evaluate_event_strategy(strategy)
                    ...
                    raw_rows.append({
                        ...
                        "selection_lower_bound": current_lower_bound,
                        "selection_upper_bound": current_upper_bound,
                        "random_requested_size": strategy_result.get(
                            "random_requested_size"
                        ),
                        "random_n_candidates_total": strategy_result.get(
                            "random_n_candidates_total"
                        ),
                        "random_chosen_size": strategy_result.get(
                            "random_chosen_size"
                        ),
                        "cap_n_candidates_total": strategy_result.get(
                            "cap_n_candidates_total"
                        ),
                        "cap_n_offline_candidates": strategy_result.get(
                            "cap_n_offline_candidates"
                        ),
                        "cap_n_online_candidates": strategy_result.get(
                            "cap_n_online_candidates"
                        ),
                        "cap_n_current_selected_online": strategy_result.get(
                            "cap_n_current_selected_online"
                        ),
                        "cap_n_current_selected_candidates": strategy_result.get(
                            "cap_n_current_selected_candidates"
                        ),
                        "cap_history_match_count": strategy_result.get(
                            "cap_history_match_count"
                        ),
                        "cap_chosen_size": strategy_result.get(
                            "cap_chosen_size"
                        ),
                        "cap_offline_chosen": strategy_result.get(
                            "cap_offline_chosen"
                        ),
                        "cap_online_chosen": strategy_result.get(
                            "cap_online_chosen"
                        ),
                        "cap_inner_constraint_count": strategy_result.get(
                            "cap_inner_constraint_count"
                        ),
                        "finite_express_exact_matches": strategy_result.get(
                            "finite_express_exact_matches"
                        ),
                        "finite_express_chosen_size": strategy_result.get(
                            "finite_express_chosen_size"
                        ),
                        "finite_express_target_size": strategy_result.get(
                            "finite_express_target_size"
                        ),
                        "finite_express_distance_backend": strategy_result.get(
                            "finite_express_distance_backend"
                        ),
                        "finite_express_max_distance": strategy_result.get(
                            "finite_express_max_distance"
                        ),
                        "finite_express_mean_distance": strategy_result.get(
                            "finite_express_mean_distance"
                        ),
                        "finite_express_relaxation_needed": strategy_result.get(
                            "finite_express_relaxation_needed"
                        ),
                        "finite_express_added_nonexact": strategy_result.get(
                            "finite_express_added_nonexact"
                        ),
                        **coverage_gap_bound_result_fields(
                            "finite_express",
                            strategy_result,
                            source_prefix="finite_express",
                        ),
                        ...
                        "relaxed_express_n_candidates_total": strategy_result.get(
                        ...
                        "relaxed_express_max_chosen_distance": strategy_result.get(
                            "relaxed_express_max_chosen_distance"
                        ),
                        "relaxed_express_mean_chosen_distance": strategy_result.get(
                            "relaxed_express_mean_chosen_distance"
                        ),
                        "relaxed_express_median_chosen_distance": strategy_result.get(
                            "relaxed_express_median_chosen_distance"
                        ),
                        "relaxed_express_min_distance": strategy_result.get(
                            "relaxed_express_min_distance"
                        ),
                        "relaxed_express_median_distance": strategy_result.get(
                            "relaxed_express_median_distance"
                        ),
                        "relaxed_express_mean_distance": strategy_result.get(
                            "relaxed_express_mean_distance"
                        ),
                        "relaxed_express_max_distance_observed": strategy_result.get(
    ...
    if relaxed_express_debug:
        relaxed_rows = [row for row in raw_rows if row["strategy"] == "RELAXED-EXPRESS"]
        if relaxed_rows:
            print("RELAXED-EXPRESS diagnostics:")
            for key in [
                "relaxed_express_chosen_size",
                "relaxed_express_exact_matches",
                "relaxed_express_max_chosen_distance",
