...
STRATEGIES = ["FINITE-EXPRESS", "EXPRESS"]
PLOT_SPECS = [
    ("FINITE-EXPRESS", "Finite EXPRESS", "#1f77b4"),
...
def focus_events(raw_df, subset):
    ...
    if subset != "finite-added-nonexact":
        raise ValueError(f"Unknown subset: {subset}")

    finite_df = raw_df[raw_df["strategy"] == "FINITE-EXPRESS"].copy()
    added_mask = bool_series(finite_df["finite_express_added_nonexact"])
    added_events = finite_df.loc[added_mask, ["run", "t"]].drop_duplicates()
...
def default_output_path(result_dir, subset):
    if subset == "finite-added-nonexact":
        return Path(result_dir) / "finite_express_added_nonexact_timeseries.png"
    return Path(result_dir) / "express_finite_express_timeseries_by_t.png"
...
def plot_timeseries(agg_df, raw_df, result_dir, output_path, subset, total_runs, inf_rule):
    ...
    if subset == "finite-added-nonexact":
        title = "FINITE-EXPRESS Nonexact-Addition Events, Averaged by Time Across Runs"
    else:
        title = "EXPRESS vs FINITE-EXPRESS, Selected Events Averaged by Time Across Runs"
...
def parse_args():
    parser = ArgumentParser(description="Plot EXPRESS and FINITE-EXPRESS metrics over time.")
    parser.add_argument(
        "--result-dir",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        "--subset",
        choices=["all", "finite-added-nonexact"],
        default="all",
