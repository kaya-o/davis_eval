...
DEFAULT_SUITE_DIR = (
    PROJECT_ROOT
    / "results"
    / "suite_20260606_225508_weighted_expr_max_dist_sweep"
)
DEFAULT_OUTPUT_NAME = "max_distance_aggregate_metrics.png"
DEFAULT_SUMMARY_NAME = "max_distance_aggregate_metrics.csv"
DEFAULT_STRATEGIES = [
    "FINITE-EXPRESS",
    "WEIGHTED-EXPRESS",
...
def max_distance_from_config(config):
    conformal = config.get("conformal", {})
    value = conformal.get("weighted_express_max_distance")
    if value is None:
        value = conformal.get("weighted_neighborhood_express_max_distance")
    if value is None:
        raise KeyError(
            "Expected conformal.weighted_express_max_distance "
            "or conformal.weighted_neighborhood_express_max_distance in run config"
        )
    return float(value)
...
def summarize_suite(suite_dir, strategies=None):
    ...
    for run_dir in result_dirs(suite_dir):
        ...
        aggregate["max_distance"] = max_distance_from_config(config)
    ...
    return summary.sort_values(["max_distance", "strategy"]).reset_index(drop=True)


def plot_summary(summary, output_path, strategies=None):
    strategies = DEFAULT_STRATEGIES if strategies is None else strategies
    max_distances = sorted(summary["max_distance"].unique())
    x = np.arange(len(max_distances))
    distance_to_x = {value: idx for idx, value in enumerate(max_distances)}

    fig, axes = plt.subplots(1, 3, figsize=(15, 4.8), sharex=True)
    for ax, (metric, ylabel) in zip(axes, METRICS):
        for strategy in strategies:
            strategy_df = summary[summary["strategy"] == strategy].sort_values("max_distance")
            if strategy_df.empty:
                continue

            style = STRATEGY_STYLES.get(strategy, {})
            ax.plot(
                [distance_to_x[value] for value in strategy_df["max_distance"]],
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([f"{value:g}" for value in max_distances])
...
def parse_args():
    parser = ArgumentParser(
        description="Plot aggregate metrics over a weighted EXPRESS max-distance sweep.",
    )
    parser.add_argument(
        "--suite-dir",
        type=Path,
        default=DEFAULT_SUITE_DIR,
        help="Suite directory containing max-distance experiment result subdirectories.",
    )
    parser.add_argument(
        "--output",
        type=Path,
