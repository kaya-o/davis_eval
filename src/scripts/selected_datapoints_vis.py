...
def plot_selected_datapoints(result_dir=None, output_path=None, run=None, y_min=None, y_max=None):
    ...
    ax.set_ylabel(r"$\hat{\mu}_2$", fontsize=15)
    ...
    handles = [
        ...
        plt.Line2D([0], [0], marker="o", color="none", markerfacecolor="#1f77b4", markersize=8),
    ]
    ax.legend(
        handles,
        ["Selection interval", r"Selected $\hat{\mu}_2$"],
        loc="lower right",
        frameon=True,
        fontsize=13,
    )
...
def plot_selected_datapoints_panels(result_dir=None, output_path=None, n_runs=8, y_min=None, y_max=None):
    ...
    time_locator = MaxNLocator(nbins=4, integer=True)
    time_ticks = time_locator.tick_values(0, x_max)
    x_axis_max = max(x_max, time_ticks[-1])
    ...
    draw_selected_datapoints_panel(
        axes[n_runs],
        mean_df,
        f"Aggregated (over {df['run'].nunique()} runs)",
        interval_alpha=0.34,
        point_alpha=0.65,
    )
