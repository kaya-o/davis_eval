...
DEFAULT_OUTPUT_NAME = "weighted_express_window_stress_gap.png"
DEFAULT_SUMMARY_NAME = "weighted_express_window_stress_gap_summary.csv"
...
def stress_summary_for_run(run_dir, recompute=False):
...
def summarize_suite(suite_dir, recompute=False):
    ...
    for run_dir in result_dirs(suite_dir):
        ...
        row = stress_summary_for_run(run_dir, recompute=recompute)
        row["window_width"] = float(config["selection"]["window_width"])
        row["run_dir"] = run_dir.name
        rows.append(row)

    summary = pd.DataFrame(rows)
    return summary.sort_values("window_width").reset_index(drop=True)
...
def parse_args():
    parser = ArgumentParser(
        description="Plot Weighted EXPRESS coverage gap and stress over a window-width sweep.",
    )
    parser.add_argument(
        "--suite-dir",
        type=Path,
