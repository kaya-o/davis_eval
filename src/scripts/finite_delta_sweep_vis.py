...
DEFAULT_OUTPUT_NAME = "finite_express_delta_sweep_panels.png"
...
def summarize_run_dir(run_dir):
    ...
    delta = config.get("conformal", {}).get("finite_express_rank_delta")
    if delta is None:
        raise ValueError(f"{run_dir} has no conformal.finite_express_rank_delta")
    delta = float(delta)
    ...
    raw_df = raw_df[raw_df["strategy"] == "FINITE-EXPRESS"].copy()
    ...
    needed_mask = bool_series(raw_df["finite_express_relaxation_needed"])
    needed_summary = summarize_run_metrics(raw_df[needed_mask].copy(), "nonexact_added")
...
def plot_delta_sweep(summary_df, save_path):
    ...
    for ax in axes[-1, :]:
        ax.set_xscale("log")
        ax.set_xlabel("finite_express_rank_delta")
        ax.set_xticks(deltas)
        ax.set_xticklabels([f"{delta:g}" for delta in deltas], rotation=35, ha="right")

    fig.suptitle("FINITE-EXPRESS Delta Sweep")
...
def parse_args():
    parser = ArgumentParser(description="Plot FINITE-EXPRESS delta-sweep metrics.")
    parser.add_argument(
        "--suite-dir",
        type=Path,
        required=True,
...
def main():
    ...
    summary_csv = args.summary_csv or suite_dir / "finite_express_delta_sweep_summary.csv"
