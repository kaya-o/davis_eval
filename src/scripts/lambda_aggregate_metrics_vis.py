...
DEFAULT_SUITE_DIR = (
    PROJECT_ROOT
    / "results"
    / "suite_20260606_205343_weighted_expr_lambda_sweep"
)
DEFAULT_OUTPUT_NAME = "lambda_aggregate_metrics.png"
DEFAULT_SUMMARY_NAME = "lambda_aggregate_metrics.csv"
DEFAULT_STRATEGIES = [
    "FINITE-EXPRESS",
    "WEIGHTED-EXPRESS",
...
def lambda_from_config(config):
    conformal = config.get("conformal", {})
    value = conformal.get("weighted_express_lambda")
    if value is None:
        value = conformal.get("weighted_neighborhood_express_lambda")
    if value is None:
        raise KeyError(
            "Expected conformal.weighted_express_lambda "
            "or conformal.weighted_neighborhood_express_lambda in run config"
        )
    return float(value)
...
def summarize_suite(suite_dir, strategies=None):
    ...
    for run_dir in result_dirs(suite_dir):
        ...
        aggregate["lambda"] = lambda_from_config(config)
...
def plot_summary(summary, output_path, strategies=None):
    ...
    lambdas = sorted(summary["lambda"].unique())
    x = np.arange(len(lambdas))
    lambda_to_x = {value: idx for idx, value in enumerate(lambdas)}

    fig, axes = plt.subplots(1, 3, figsize=(15, 4.8), sharex=True)
    for ax, (metric, ylabel) in zip(axes, METRICS):
        for strategy in strategies:
            strategy_df = summary[summary["strategy"] == strategy].sort_values("lambda")
            if strategy_df.empty:
                continue

            style = STRATEGY_STYLES.get(strategy, {})
            ax.plot(
                [lambda_to_x[value] for value in strategy_df["lambda"]],
...
def parse_args():
    parser = ArgumentParser(
        description="Plot aggregate metrics over a weighted EXPRESS lambda sweep.",
    )
    parser.add_argument(
        "--suite-dir",
        type=Path,
        default=DEFAULT_SUITE_DIR,
        help="Suite directory containing lambda experiment result subdirectories.",
    )
    parser.add_argument(
        "--output",
        type=Path,
