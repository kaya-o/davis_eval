...
def summarize_strategy_run_dir(run_dir, bin_width, strategy, k_value):
...
def summarize_run_dir(run_dir, bin_width):
    ...
    return summarize_strategy_run_dir(
        run_dir,
        bin_width=bin_width,
        strategy=STRATEGY,
        k_value=int(k_value),
...
def summarize_express_run_dir(run_dir, bin_width):
    return summarize_strategy_run_dir(
        run_dir,
        bin_width=bin_width,
        strategy=EXPRESS_STRATEGY,
        k_value=pd.NA,
    )
...
def plot_summary(summary, output_path):
    k_values = sorted(summary["k"].dropna().unique())
    panels = [
        (f"k={int(k_value)}", summary[summary["k"].eq(k_value)])
        for k_value in k_values
    ]
    express_panel = summary[summary["k"].isna()]
    if not express_panel.empty:
        panels.append((EXPRESS_STRATEGY, express_panel))

    fig, axes = plt.subplots(3, 4, figsize=(17, 9.5), sharex=True)
    axes_flat = list(axes.flat)
    if len(panels) > len(axes_flat):
        raise ValueError(
            f"The 3x4 layout supports at most {len(axes_flat)} panels, "
            f"found {len(panels)}"
        )
    ...
    legend_handles = None
    legend_labels = None
    for panel_index, (ax, (title, panel)) in enumerate(zip(axes_flat, panels)):
        panel = panel.sort_values("t_bin")
        ax2 = ax.twinx()
...
def build_outputs(
    ...
    if include_express:
        express_summary = summarize_express_run_dir(
            discover_express_run_dir(suite_dir),
            bin_width=bin_width,
        )
        summary = pd.concat([summary, express_summary], ignore_index=True)
    summary = summary[
        [
            "k",
            "run_dir",
...
def main():
    ...
    panel_count = summary["k"].nunique() + int(summary["k"].isna().any())
    print(f"panels={panel_count}, bin_width={args.bin_width}")
