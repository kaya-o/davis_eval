...
DEFAULT_OUTPUT_NAME = "k_sweep_metrics_4panels.png"
METRICS = [
    ("infinite_fraction", "Infinite Fraction"),
    ("median_interval_length", "Meadian Interval Length"),
    ("miscoverage", "Miscoverage"),
    ("avg_n_calibration", "Average Calibration\nSet Size"),
]
...
def summarize_k_sweep(suite_dir):
    ...
    if len(express_rows) != 1:
        raise ValueError(
            f"Expected exactly one EXPRESS row under {suite_dir}, found {len(express_rows)}"
        )

    return pd.DataFrame(k_rows).sort_values("k"), express_rows[0]
...
def plot_k_sweep_4panel(suite_dir=DEFAULT_SUITE_DIR, output_path=None):
    suite_dir = Path(suite_dir)
    k_df, baseline = summarize_k_sweep(suite_dir)

    if output_path is None:
        output_path = suite_dir / "vis" / DEFAULT_OUTPUT_NAME
    else:
    ...
    fig, axes = plt.subplots(4, 1, figsize=(10, 11.4), sharex=True)
    ...
    fig.suptitle(r"Performance Metrics with Varying $k$", fontsize=16)
    ...
    pd.concat([k_df, pd.DataFrame([baseline])], ignore_index=True).to_csv(
        summary_path,
        index=False,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot K-EXPRESS k-sweep metrics with avg_n_calibration."
    )
    parser.add_argument("--suite-dir", type=Path, default=DEFAULT_SUITE_DIR)
    parser.add_argument("--output", type=Path, default=None)
    args = parser.parse_args()

    output_path, summary_path = plot_k_sweep_4panel(
