...
THREE_PANEL_OUTPUT_NAME = "adaptive_weighted_express_timeseries_3panels_run_binned_clipped_nlow.png"
THREE_PANEL_SUMMARY_NAME = "adaptive_weighted_express_timeseries_3panels_run_binned_clipped_nlow.csv"
THREE_PANEL_RUN_BIN_MEANS_NAME = "adaptive_weighted_express_timeseries_3panels_run_bin_means_clipped_nlow.csv"
...
def plot_summary(summary, output_path, result_dir, target_count, midpoint_count, slope, plot_specs=None):
    del result_dir
    plot_specs = PLOT_SPECS if plot_specs is None else plot_specs
    fig_height = 3.25 * len(plot_specs)
    fig, axes = plt.subplots(len(plot_specs), 1, figsize=(14, fig_height), sharex=True)
...
def build_outputs(result_dir, output_dir=None, bin_width=250, omit_stress_input_panel=False):
    ...
    if omit_stress_input_panel:
        plot_specs = THREE_PANEL_PLOT_SPECS
        run_bin_means_path = output_dir / THREE_PANEL_RUN_BIN_MEANS_NAME
        summary_path = output_dir / THREE_PANEL_SUMMARY_NAME
        output_path = output_dir / THREE_PANEL_OUTPUT_NAME
    else:
...
def parse_args():
    ...
    parser.add_argument(
        "--omit-stress-input-panel",
        action="store_true",
        help="Generate the same binned diagnostics figure without the bottom stress-input-count panel.",
    )
...
def main():
    ...
    output_path, summary_path, run_bin_means_path, summary = build_outputs(
        ...
        omit_stress_input_panel=args.omit_stress_input_panel,
    )
