...
TARGET_ALPHA = 0.4
FINITE_NEEDED_KEY = "FINITE-EXPRESS-NEEDED"
STRATEGY_ORDER = [
    "FULL",
    "S-FIX",
    "S-FULL",
...
def strategy_label_overrides_from_config(config):
    ...
    labels = {
        FINITE_NEEDED_KEY: "FINITE-EXPRESS\n(nonexact added)",
    }
...
def finite_needed_run_means(raw_df):
    if "finite_express_relaxation_needed" not in raw_df.columns:
        raise ValueError("raw_selected_events.csv is missing finite_express_relaxation_needed")

    finite_df = raw_df[raw_df["strategy"] == "FINITE-EXPRESS"].copy()
    needed_df = finite_df[bool_series(finite_df["finite_express_relaxation_needed"])].copy()
    if needed_df.empty:
        raise ValueError("No FINITE-EXPRESS rows with finite_express_relaxation_needed=True")

    needed_df["strategy"] = FINITE_NEEDED_KEY
    return needed_df


def build_metrics(raw_df):
    standard_metrics = summarize_run_means(raw_df)
    needed_metrics = summarize_run_means(finite_needed_run_means(raw_df))
...
def plot_sale_ramdas_finite_needed(
    metrics_df,
    save_path,
    title,
    ...
    if FINITE_NEEDED_KEY in strategies:
        needed_x = strategies.index(FINITE_NEEDED_KEY)
...
def parse_args():
    parser = ArgumentParser(
        description="Sale-Ramdas-style plot with a nonexact-added FINITE-EXPRESS column."
    )
    parser.add_argument(
        "--result-dir",
        type=Path,
...
def main():
    ...
    output_path = args.output or result_dir / "sale_ramdas_finite_needed_summary.png"

    plot_sale_ramdas_finite_needed(
        metrics_df,
        output_path,
        title=None if args.hide_title else f"Sale-Ramdas Style Finite Summary - {result_dir.name}",
        parameter_text=args.parameter_text,
        strategy_labels=strategy_labels,
    )

    needed_events = raw_df[
        (raw_df["strategy"] == "FINITE-EXPRESS")
        & bool_series(raw_df["finite_express_relaxation_needed"])
    ]
    print(f"Wrote summary plot to {output_path}")
    print(
        "FINITE-EXPRESS nonexact-added events: "
